...
from core.levels.sandbox_level import SandboxLevel
        ...
        if self.rect.collidepoint(*pygame.mouse.get_pos()):
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if self.text == 'Exit':
                    ...
                    return
                elif self.text == 'Sandbox':
                    SandboxLevel(1280, 720)
                    return
                elif self.text == 'Skill Tree':
                    # Open skill tree from main menu
                    from core.menu.main import TheCageOfMage
                    skill_tree_ui = SkillTreeUI(
                        pygame.display.get_surface(),
                        'font/Chalk_and_Pamor.ttf',
                        lambda: TheCageOfMage(1280, 720)
                    )
                    skill_tree_ui.show()
                    while skill_tree_ui.active:
                        for evt in pygame.event.get():
                            skill_tree_ui.handle_event(evt)
